...
$(() => {
  ...
  // Only run form manipulation if not in readonly mode
  const isReadonly = $(".chal-function").prop("disabled");
  if (!isReadonly) {
    $(".chal-function")
      .change(function () {
  ...
  if (document.querySelector("#challenge-flags")) {
    ...
    new flagList({
      propsData: { challenge_id: window.CHALLENGE_ID, readonly: true },
    }).$mount(vueContainer);
  }
